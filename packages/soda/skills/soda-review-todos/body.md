...
| `wat node get <id>` | Retrieve a node with all its relations (tags, links) |
| `wat list kinds` | List all node kinds with counts |

## Instructions
1. Fetch all TODOs: `wat node search --kind todo`
2. If tags provided, filter: `wat node search --kind todo --tags <tag1>,<tag2>`
...
4. For each pending TODO, discuss with user:
   ...
   - Status update if needed → `wat node update <id> --prop status=in_progress --prop priority=high`
