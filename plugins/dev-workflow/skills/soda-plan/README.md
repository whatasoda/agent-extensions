...
- **Separated confirmation gates**: Direction Confirmation and Branch Strategy are presented as two sequential AskUserQuestion interactions within Step 2. The original design merged them into a single interaction to reduce round-trips, but this caused direction-related options (方針を調整, さらに調査を深める) to mix with branch options (新ブランチ作成, 現ブランチ続行), making each question's purpose unclear. The separated design (Step 2a: direction, Step 2b: branch) keeps each interaction focused on a single orthogonal concern. The branch question is lightweight (2 options), so the extra round-trip cost is minimal. The step also includes an Investigation Digest block — presenting task scale classification (with evidence), investigation summary, and anticipated design decisions (with confidence indicators) — to surface the AI's autonomous judgments before the user confirms direction. S-task invocations use a condensed single-line digest to minimize friction. Step 2a uses context-dependent options: when a Proposal Summary exists, approach re-selection is omitted (already decided); when absent, a "/soda-propose" escalation option and a "さらに調査を深める" middle-ground option are included.
- **Deeper investigation option**: The no-Proposal-Summary path includes a "さらに調査を深める" option (max 1 round) as a middle ground between accepting the current investigation and escalating to `/soda-propose`. This addresses the gap where investigation was insufficient but a full approach comparison is unnecessary. The hard cap of 1 round limits worst-case interaction increase to +1 and prevents unbounded re-investigation loops.
- **Investigation Digest approach boundary**: In the no-Proposal-Summary path, the Investigation Digest's anticipated design decisions are constrained to implementation-level decisions only. Approach comparisons and alternative strategies are explicitly prohibited — they belong to `/soda-propose`. When investigation findings raise fundamental scope or premise questions, the digest recommends `/soda-propose` escalation via a forced trigger rather than embedding approach-level analysis. This preserves the separation of concerns between the two skills.
