...
- **Merged confirmation gates**: Strategy Confirmation and Branch Strategy are merged into a single AskUserQuestion interaction. The original design had them as separate steps, but sequential AskUserQuestion calls for closely related decisions created unnecessary round-trips and contributed to the perception of duplicate questioning. The merged step now also includes an Investigation Digest block — presenting task scale classification (with evidence), investigation summary, and anticipated design decisions (with confidence indicators) — to surface the AI's autonomous judgments before the user confirms direction. This enrichment adds zero net interaction for the base case (it augments the existing mandatory gate) while enabling early misalignment detection. S-task invocations use a condensed single-line digest to minimize friction. The merged step uses context-dependent options: when a Proposal Summary exists, approach re-selection is omitted (already decided); when absent, a "/soda-propose" escalation option and a "さらに調査を深める" middle-ground option are included.
